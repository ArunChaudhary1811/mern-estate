...
export default function UpdateListing(){
    ...
    const handleImageSubmit = (e) => {
        if(files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            if(files.length > 0) setImageUploadError("You can only upload 6 images per listing");
            else setImageUploadError("No image choosen");
        }
    };
                    ...
                    <p className="text-red-700 text-sm">{imageUploadError && imageUploadError}</p>
                    {
                        formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => {
                            return <div key={url} className="flex justify-between p-3 border items-center">
                                <img src={url} alt="Listing image" className="w-20 h-20 object-content rounded-lg" />
